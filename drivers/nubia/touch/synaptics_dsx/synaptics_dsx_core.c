...
static int synaptics_request_power_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (bdata->power1_gpio >= 0) {
                pr_info("%s:start power off 3v3\n", __func__);
                retval = synaptics_rmi4_gpio_setup(
                                bdata->power1_gpio,
                                true, 1, !bdata->power1_on_state);
                if (retval < 0) {
                        dev_err(rmi4_data->pdev->dev.parent,
                                        "%s: Failed to configure power GPIO\n",
                                        __func__);
		...
		bool enable)
{
	...
	if (bdata->power1_gpio >= 0) {
			gpio_set_value(bdata->power1_gpio, bdata->power1_on_state);
			msleep(5);
		}
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	retval = drm_panel_notifier(&rmi4_data->fb_notifier);
	if (retval < 0) {
		dev_err(&pdev->dev,
